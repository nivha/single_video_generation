...
def get_args():
    ...
    parser.add_argument('--results_dir', default='./result', help='where to save the results')
    parser.add_argument('--a_frames_dir', help='path to spatio-temporal layout video (see README for video format)')
    parser.add_argument('--b_frames_dir', help='path to appearance video (see README for video format)')
    parser.add_argument('--a_n_bins', type=int, default=None, help='number of bins for synamic structure')
    parser.add_argument('--b_n_bins', type=int, default=None, help='number of bins for synamic structure')

    parser.add_argument('--a_start_frame', default=1, help='use this #d.png as first frame in the video', type=int)
    parser.add_argument('--a_end_frame', default=30, help='use this #d.png as first frame in the video', type=int)
    parser.add_argument('--a_max_size', default=256, help='maximal spatial size (same as in SinGAN)', type=int)
    parser.add_argument('--b_start_frame', default=1, help='use this #d.png as first frame in the video', type=int)
    parser.add_argument('--b_end_frame', default=30, help='use this #d.png as first frame in the video', type=int)
    parser.add_argument('--b_max_size', default=256, help='maximal spatial size (same as in SinGAN)', type=int)

    parser.add_argument('--downfactor', default='(0.9,0.9)', type=str2list, help='downscale factor of the pyramid (T,HW)')
    parser.add_argument('--min_size', default='(3,20)', type=str2list, help='shape of the coarsest pyramid level (T,HW)')
    parser.add_argument('--kernel_size', default='(3,5,5)', type=str2list, help='')
    parser.add_argument('--J', default=1, help='number of EM-iterations within level', type=int)
    parser.add_argument('--reduce', default='median', help='patchmatch fold (vote) function', choices=['mean', 'median'])
    parser.add_argument('--alpha', default=1, type=float, help='alpha for completion score')
    parser.add_argument('--n_max_flow', default=0.5, type=float, help='maximal level of the pyramid for using dynamic structure (between 0-1)')
